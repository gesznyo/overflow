...
app.MapGet("/search", async (string query, ITypesenseClient client) =>
{
    ...
    var searchParams = new SearchParameters(text: query, queryBy: "title,content");
    ...
    try
    {
        var result = await client.Search<SearchQuestion>(collection: "questions", searchParams);
...
{
    var searchParams = new SearchParameters(text: query, queryBy: "title");
    try
    {
        var result = await client.Search<SearchQuestion>(collection: "questions", searchParams);
        return Results.Ok(result.Hits.Select(hit => hit.Document));
